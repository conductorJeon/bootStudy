...
public interface BusanTravelRepository extends JpaRepository<BusanTravel, Integer> {
	@Query(value = "SELECT contentid, title, address, image1, hit, contenttype, rownum " + "FROM busantravel "
			+ "ORDER BY hit DESC " + "OFFSET 0 ROWS FETCH NEXT 4 ROWS ONLY", nativeQuery = true)
